// e-commerce/frontend/components/layout/SharedNavbar.tsx
"use client";
...
import { Menu, X, ShoppingCart, User, Search, ShoppingBag, Home, Package, Info, Phone, LogOut } from "lucide-react";
...
export default function SharedNavbar() {
  ...
  const { user, profile, signOut, cartTotalItems, isAddingToCart, clearCartAnimation } = useAuth();
  ...
  const linkClass = (href: string) => {
    const isActive = pathname === "/" && href.includes("#") ? 
      false : pathname === href.replace("/#", "");
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            >
              ...
              <span className={`absolute -top-2 -right-2 bg-red-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center transition-all duration-300 ${
                  ...
                  <button className="outline-none">
                    <Avatar className="h-10 w-10 cursor-pointer border-2 border-red-600/50 hover:border-red-500 transition-colors">
                      ...
                      <AvatarFallback className="bg-red-700 text-white font-semibold">
                        {profile?.fullName ? profile.fullName.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : user.email?.charAt(0).toUpperCase() || 'U'}
                      </AvatarFallback>
                    </Avatar>
                  </button>
                </DropdownMenuTrigger>
                ...
                >
                  <DropdownMenuLabel className="text-gray-900">
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-semibold">{profile?.fullName || 'User Account'}</p>
          ...
          <div className="md:hidden mt-4 border-t border-red-800/30 pt-4">
            <div className="space-y-2">
              {navigation.map((item) => {
                ...
                return (
                  <button
                    key={item.name}
                    onClick={() => handleNavigation(item.href)}
                    className="text-red-200 hover:bg-red-800 hover:text-white px-4 py-2 rounded-lg transition-all duration-200 cursor-pointer flex items-center gap-2 w-full text-left"
                  >
                    <Icon className="w-4 h-4" />
                    {item.name}
                  </button>
                ...
                {user ? (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <div className="w-full">
                        <div className="text-red-200 text-sm px-4 py-2 flex items-center justify-between cursor-pointer bg-red-800/30 rounded-lg">
                          <span>Hi, {profile?.fullName || user.email?.split('@')[0]}</span>
                    ...
                    >
                      ...
                      {/* TAMBAH INI: My Profile Menu Item untuk Mobile */}
                      <DropdownMenuItem
                        onClick={() => {
                          router.push("/profile");
                          setIsMenuOpen(false);
                  ...
                  <>
                    <Link href="/login" onClick={() => setIsMenuOpen(false)}>
                      <Button size="sm" className="w-full bg-red-700 hover:bg-red-600 text-white">
                        Login
                      </Button>
                    </Link>
              ...
              <div className="px-4 py-2">
                ...
                >
                  <ShoppingCart className={`w-4 h-4 mr-2 transition-all duration-300 ${
                    isCartAnimating ? 'animate-bounce' : ''
                  }`} />
                  Cart
                  <span className={`absolute -top-2 -right-2 bg-red-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center transition-all duration-300 ${
